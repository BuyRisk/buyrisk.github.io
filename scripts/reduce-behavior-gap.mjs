...
async function main() {
  ...
  for (const f of equityFunds.filter((f) => f.months >= 120 && f.w >= 200 && f.gap > 0 && /[a-z]/i.test(f.name)).sort((a, b) => b.gap - a.gap)) {
    ...
    seen.add(key);
    // ANONYMIZED pending the WRDS licence-scope answer: describe by era/category,
    // never by name (map new cases here; fallback keeps them nameless).
    const ANON = {
      "Allianz Funds: Allianz RCM Innovation Fund": "A dot-com era innovation fund",
      "Firsthand Funds: Technology Value Fund": "A 1990s technology fund",
      "ARK ETF Trust: ARK Genomic Revolution ETF": "A 2020s thematic ETF",
      "Munder Series Trust: Munder Growth Opportunities Fund": "A late-90s growth fund",
      "Allianz Funds: RCM Biotechnology Fund": "A biotech sector fund",
      "Old Mutual Funds II: Old Mutual Select Growth Fund": "A concentrated growth fund",
    };
    cases.push({ name: ANON[display] ?? `A high-volatility fund (case ${cases.length + 1})`, tw: round(f.tw, 4), dw: round(f.dw, 4), gapPP: round(f.gap, 4), years: `${f.startY}–${f.endY}` });
