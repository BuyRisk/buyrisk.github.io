...
async function main() {
  ...
  const funds = picked.map((f, i) => ({
    // ANONYMIZED pending the WRDS licence-scope answer: publish "Fund A"…"Fund L"
    // (ordered by assets), never the real names. The real name is logged to the
    // console for local verification only.
    name: `Fund ${String.fromCharCode(65 + i)}`,
    _localName: prettify(f.name),
